...
const GiftCouponCard = () => {
  ...
  // Fetch latest coupon when component mounts
  useEffect(() => {
    getMyCoupon();
  }, [getMyCoupon]);
    ...
    >
      ...
      {coupon && !isCouponApplied && (
        <div className="mt-4 p-4 bg-gray-900 rounded-lg border border-gray-700">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={handleApplyMyGiftCoupon}
          >
            Apply this Coupon
          </motion.button>
        </div>
      ...
      {isCouponApplied && coupon && (
        <div className="mt-4">
          ...
          <motion.button
            type="button"
            className="mt-2 w-full rounded-lg bg-red-600 py-2.5 text-sm font-medium text-white hover:bg-red-700"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
